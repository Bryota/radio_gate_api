...
class RadioProgramController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            Log::error('【管理者】ラジオ番組一覧がありませんでした。', ['error' => $e, 'request' => $request]);
            return response()->json([
                'message' => '該当のデータが見つかりませんでした。'
            ], 500, [], JSON_UNESCAPED_UNICODE);
        } catch (\Throwable $th) {
            Log::error('【管理者】ラジオ番組一覧取得エラー', ['error' => $th, 'request' => $request]);
            return response()->json([
    ...
    public function show(int $radio_program_id)
    {
        try {
            ...
            Log::error('【管理者】ラジオ番組がありませんでした。', ['error' => $e]);
            return response()->json([
                'message' => '該当のデータが見つかりませんでした。'
            ], 500, [], JSON_UNESCAPED_UNICODE);
        } catch (\Throwable $th) {
            Log::error('【管理者】ラジオ番組取得エラー', ['error' => $th]);
            return response()->json([
    ...
    public function store(RadioProgramRequest $request)
    {
        try {
            ...
            Log::error('【管理者】ラジオ番組作成エラー', ['error' => $th, 'request' => $request]);
            return response()->json([
    ...
    public function update(RadioProgramRequest $request, int $radio_program_id)
    {
        try {
            ...
            $this->db_connection->rollBack();
            Log::error('【管理者】ラジオ番組更新エラー', ['error' => $th, 'request' => $request]);
            return response()->json([
    ...
    public function destroy(int $radio_program_id)
    {
        try {
            ...
            Log::error('【管理者】ラジオ番組削除エラー', ['error' => $th]);
            return response()->json([
